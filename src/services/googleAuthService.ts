...
if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET || !GOOGLE_REDIRECT_URI) {
    // Bu bir sunucu uygulaması olduğu için hata fırlatmak daha doğru.
    ...
    console.warn("UYARI: Google kimlik bilgileri (.env dosyasında GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI) eksik. Google entegrasyonu çalışmayacak.");
}
...
export async function getGoogleTokens(code: string): Promise<void> {
    ...
    if (tokens.refresh_token) {
        ...
        // process.env'i de güncelleyelim ki hemen kullanılabilsin
